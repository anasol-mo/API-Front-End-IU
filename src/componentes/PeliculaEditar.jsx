...
function PeliculaEditar() {
  ...
  const fetchGeneros = async () => {
    ...
    try {
      ...
      const response = await fetch(`${url_api}/generos`)
      const data = await response.json()
    ...
    <>
      <section className='peli-edit-section' >
        ...
        <form onSubmit={onSubmit}>
          ...
          <p>
            <label htmlFor="portada">Enlace Imagen de Portada </label>
            <input type="url" {...register("portada", { required: true })} id="portada" />
          </p>
          <p>
          ...
          <p>
            ...
            <select id="director" {...register("director", { required: true })}   >
              {directores.map(director => <option key={director._id} value={director._id}>{director.nombres}</option>)}
            </select>
          </p>
          <p>
